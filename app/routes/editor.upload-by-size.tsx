            ...
            <>
              ...
              <div className="meta">
                ...
                {activeQuote?.lowDpi || (activeQuote && activeQuote.effectiveDpi < 200) ? (
                  <span className="warn">
                    Effective print resolution ~{activeQuote?.effectiveDpi ?? "?"} DPI — upload a
                    higher-resolution file for best results.
                  </span>
                ) : null}
              </div>
              ...
              <div className="fields">
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      patchActive({ quantity: Math.max(1, parseInt(e.target.value, 10) || 1) })
                    }
                  />
                </label>
              </div>
...
const CSS = `
.ubs{--blue:#2463eb;--green:#21a366;min-height:100vh;background:#eef1f5;color:#111827;font:14px/1.35 Inter,system-ui,sans-serif}
.ubs button,.ubs input,.ubs select,.ubs label{font:inherit}
...
.btn.primary{background:var(--blue);color:#fff;display:inline-grid;place-items:center}
...
.queue{padding:14px}
.queue h2,.summary h2{margin:0 0 10px;font-size:15px}
...
.queue li button.active{border-color:#7aa2f8;background:#edf3ff}
.queue img{width:48px;height:48px;object-fit:contain;background:#eee}
...
.preview img{max-width:100%;max-height:220px;object-fit:contain;background:#fff;border:1px solid #dfe3e8;border-radius:8px;padding:8px}
.meta{display:grid;gap:4px}.meta span{font-size:12px;color:#667085}.warn{color:#b45309;font-size:12px}
.fields{display:grid;grid-template-columns:1fr 1fr;gap:10px}
.fields label{display:grid;gap:5px;font-size:12px;color:#667085}
.fields input,.fields select{padding:8px;border:1px solid #ccd2da;border-radius:6px}
.check{grid-column:1/-1;flex-direction:row;align-items:center;display:flex!important;gap:8px;color:#111}
