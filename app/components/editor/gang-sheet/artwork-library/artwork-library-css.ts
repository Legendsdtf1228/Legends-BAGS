...
.lgs-artlib-lead{margin:0 12px 6px;font-size:10px;color:var(--lgs-muted);line-height:1.35}
.lgs-artlib-drop{
...
.lgs-artlib-grid{
  display:grid;grid-template-columns:1fr;gap:6px;padding:0 12px 10px;
...
.lgs-artlib-card{
  ...
  padding:6px;background:var(--lgs-ink);border:1px solid var(--lgs-line);border-radius:8px;min-width:0;max-width:100%
}
.lgs-artlib-card.on-sheet{border-color:var(--lgs-gold-deep);box-shadow:inset 0 0 0 1px var(--lgs-gold-deep)}
.lgs-artlib-thumb{
  position:relative;display:block;width:76px;height:76px;padding:0;border:0;border-radius:5px;overflow:hidden;cursor:pointer;background:#0a0c10;flex-shrink:0
}
...
.lgs-artlib-badge{
  position:absolute;left:3px;bottom:3px;font-size:8px;font-weight:800;font-style:normal;line-height:1;padding:2px 4px;border-radius:3px;
  background:#0d1117cc;color:#fff
}
.lgs-artlib-badge.on-sheet{background:var(--lgs-gold);color:var(--lgs-ink)}
.lgs-artlib-body{display:grid;gap:4px;min-width:0;min-height:min-content}
.lgs-artlib-name{
  font-size:11px;font-weight:600;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;line-height:1.2
}
.lgs-artlib-meta{display:flex;flex-wrap:wrap;gap:2px 7px;font-size:10px;color:var(--lgs-muted);line-height:1.2}
.lgs-artlib-meta b{color:var(--lgs-soft);font-weight:700}
.lgs-artlib-meta .dpi-badge.tier-excellent{color:#86efac}
.lgs-artlib-meta .dpi-badge.tier-good{color:#7dd3fc}
.lgs-artlib-meta .dpi-badge.tier-low{color:#fcd34d}
.lgs-artlib-meta .dpi-badge.tier-poor,.lgs-artlib-meta .dpi-badge.tier-unknown{color:#fca5a5}
.lgs-artlib-addrow{display:grid;grid-template-columns:1fr;gap:4px;align-items:stretch}
.lgs-artlib-qty{
  display:grid;grid-template-columns:22px 28px 22px;align-items:center;border:1px solid var(--lgs-line);border-radius:5px;overflow:hidden;background:var(--lgs-panel-2);width:max-content
}
...
.lgs-artlib-qty input::-webkit-outer-spin-button,.lgs-artlib-qty input::-webkit-inner-spin-button{appearance:none;margin:0}
.lgs-artlib-add{
  border:0;border-radius:5px;background:var(--lgs-gold);color:var(--lgs-ink);
  font-size:12px;font-weight:800;letter-spacing:.04em;text-transform:uppercase;cursor:pointer;padding:0 8px;line-height:1;white-space:nowrap;height:30px;min-height:30px;width:100%;flex-shrink:0
}
.lgs-artlib-add:hover:not(:disabled){background:#ffe28a}
.lgs-artlib-add:disabled{opacity:.45;cursor:not-allowed}
.lgs-artlib-toolsrow{display:flex;gap:4px;align-items:center;min-width:0}
.lgs-artlib-toolsrow input[type=text]{
  flex:1;min-width:0;width:0;height:24px;padding:0 6px;border:1px solid var(--lgs-line);border-radius:4px;
